...
module.exports.playLeadMelody = function () {
  var note1 = notes[_.random(0, notes.length - 1, false)];
  var note2 = notes[_.random(0, notes.length - 1, false)];
  var note3 = notes[_.random(0, notes.length - 1, false)];

  if (_.random(0, 10) > 5) note3 = null;
  if (_.random(0, 10) > 7) note2 = null;

  [note1, note2, note3].forEach(function (note, index) {
    if (!note) return;
    var now = context.currentTime;
    var offset = (index * _.random(0.3, 0.6));
    piano.play(note, now + offset);
    newGuitar.play(note, now + offset + _.random(-0.1, 0.1));
    dream.play(note, now + offset + _.random(-0.1, 0.1));
  });
